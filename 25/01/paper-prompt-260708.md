...
例 3（非增量工作，多用流程图或数据 chain）:
1. img & text -> [encoder] -> [VLM]
2. noisy action -> [Linear] -> [flow matching(action expert)]
3. [VLM] <--MOT 交互--> [action expert] -> action velocity
...
保持简短。 增量工作指出4个以内增量，非增量工作仅保留8个以内重要模块.
...
例子: raw action -> [Linear] -> action tokens -> [some module] -> some data 其中，数据不用包裹，可学习模块用 [] 包裹起来.
