...
使用 arxiv api 阅读论文，写入极简概述到执行的 md（未指定则覆盖 ~/tmp/1.md），写入后无需再跟我解释论文，回答“已完成”即可.
...
<空行>
正文第一段 0-4 个逗号保持极简，说动机（为了解决什么问题）、贡献，可选对比前作的新增，在限制句数中让小白都能 get 到如何去复现这篇论文。
<空行>
...
不要为论文辩护，不要引用论文自己的营销词来当评价，论文新提出的组件和名词如要引用必须括号解释，如果涉及网络架构则从架构角度解释. e.g. 用 Video Former(新，基于 attn 和 FFN 将视频 latent 提取为固定数量 token)
