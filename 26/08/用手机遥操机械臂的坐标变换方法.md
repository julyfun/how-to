...
收到第一帧 iPhone 变换时，这个变换变量叫做 a0（用 class 管理状态）

在这个瞬间，读取机械臂 TCP 位姿，存为变量 `tcp0_to_base`，（TCP 第 0 帧在机械臂 base 下的变换）

后续任意时刻，我们会收到 iPhone 的变换，变量叫做 `a_t`
...
计算 `b_to_tcp = tcp0_to_base.inverse() * b0_to_base`，这是一个纯旋转矩阵.

然后 tcp_t_to_base = tcp0_to_base.inverse() * b_to_tcp * a_t * b_to_tcp.inverse()，算出来了后参考合适的非夕机械臂 API，让机械臂运动。
...
其实上面说的 a_t 已经是一个变换了，不用存储 a0，然后每一帧拿到的数据称为 "at_to_a0" 好了
