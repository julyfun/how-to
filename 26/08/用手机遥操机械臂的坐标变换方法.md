...
实验表明，这非常 work. 下面基本就是当时给 codex 的 prompt. (实际实验有个小失误，iphone 端直接发送 a_t_to_a0，感觉没啥必要)

---

在仓库中，scripts/ 下，新建脚本。需要的 API 可以多参考仓库下的文件
...
在存储 a0_to_iphone_base 和 tcp0_to_base 的时候，我们定义变换 `b0_to_base`，其 position = tcp0_to_base.position，其 rotation 为单位矩阵.
...
iphone 有时候过程中停止发送，也可能网络卡顿，如果超过 1 秒，接下来收到时视为新的第一帧。如有不确定信息，问我. iphone 发的包带 session_id，session_id 变化时也是新的一帧（a0_to_iphone_base 重新记录）

## 给 Ai 的附录：

### server 代码参考如下，session_id 等尚不完整
