...
- Xcode 的防开发者设计：隐藏编译信息，用不是二进制也不像文本的 xcodeproj 格式管理项目（防 git 设计），它的 debug 窗口不像一般的终端而是 0.3s 刷新一次(Why??)
- 包很少，三方包质量很差，fetch 对网络要求高，构建系统稀烂，即使简单几个包也会冲突，有的包为了非常少用的功能放弃 Swift 5
- iOS 的不同版本竟然会影响 Swift 语言特性，比如 iOS 16 的 onChange 和 iOS 17 的 onChange 函数参数不兼容
- Swift 编译很慢
- 全局 import 是一种防开发者设计
- Coding Agent 写 Swift 最终会写出一坨构式
- Cache 很大，而苹果的硬盘和金子一样贵
- 互联网信息过时，Google 和 Gemini 能给你的方案麻烦又折腾
