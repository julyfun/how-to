...
- Swift 不严格的竞态检查，例如在非主线程写入 UI 变量有 99.95% 的概率告诉你一个警告和 0.05% 的概率 crash，导致开发期找不出 bug，上线了一个劲崩溃。并且我至今没有发现如何开这些警告 backtrace，你只知道这个项目里有警告却不知道是哪行代码引起的
- 两百多关键字，海量酸爽隐式语言特性，比如 @ObservedObject 不可嵌套，你嵌套了也没有任何报错和警告，在某一天你会发现它影响 UI 更新，然后在某个博客里读到不可嵌套。在注释和官方文档里都没有提这些语言特性
- 构式的官方文档比其他语言 std 注释还短，全靠开发者论坛云里雾里全程摸黑，并且苹果论坛很快就会关闭帖子. 苹果工程师的回答敷衍了事，苹果开发者的反馈渠道 ¥688 年度 VIP 才可开启（入口藏的很深），然而只提供行政支持（这是什么玩意也叫开发者反馈？）技术上基本不可能有回复.
- 不充 688 VIP 连安装到手机都会有 Bug "Your Development Team Has Reached Maximum Registered iPhone Devices"
- 构式的内置框架，如多点通信会随机断连，开发者嗷嗷叫苦
