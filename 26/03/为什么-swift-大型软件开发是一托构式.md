...
7. 包数量少，patterns、网络、算法、容器、序列化方向三方包质量很差，即使简单几个包也会冲突，有的包为了非常少用的功能放弃 Swift 5，而中等项目 swift 5 迁移到 swift 6 会多几百个编译错误，并分裂了包生态. swift 6.0 -> 6.1 -> 6.2 -> 6.3 经常性破坏性变更，而 xcodeproj 中的 swift version 被 xcode version 绑定，xcode version 被 macos version 绑定，你带的新人的开发机自动更新了 macos 就不得不求助于你
8. target iOS 的不同版本会影响许多官方框架特性，比如 iOS 16 的 onChange 和 iOS 17 的 onChange 函数参数破坏性变更
...
14. swift 十分鼓励隐式控制流，但对历史遗留语法缺乏 edition 和 deprecated 机制，e.g. getter-setter, property wrappers, computer property, swiftui DSL, optional chaining, 混合 exception 和没有错误传播符的 Result，而官方包仍广泛使用 exception，尽管 rust 的 unwrap() 已经让人畏惧，而 swift 用 `Type!` 和 `variable!` 轻松 panic，你的新人如果不经你的监督和你强大的子集约束，用 GPT5.5 / Opus4.8 写 Swift 最终仍会写出海量构式，构造史山的能力显著强于其他语言
15. DerivedData, Module Cache, Device Cache 很大，而苹果的硬盘很贵，平台 ci 对 macos 的定价通常比其他平台多一个数量级
