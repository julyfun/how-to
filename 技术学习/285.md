...
$r$ 导数怎么传递到 $theta$ 上的？
- Q-learning 是 (s, a) $- theta ->$ r，这里 s,a,r 都在数据集中，所以就是监督学习
...
- Policy Gradient: 求 $EE[r]$ 对 $theta$ 的导数.
    - $nabla_theta J(theta) tilde.eq EE[nabla_theta log pi_theta (a|s)A(s, a)]$
	- 训练:
		- Actor: 输入 `[机械臂状态，观测]`
		- 输出 `[动作]` 或者 `[动作的概率分布]`
	- 推理: 一样
...
- 上图两种训练方法。选择 $alpha$: natural gradient. 选择 $epsilon$: trust region policy optimization
- Lec5 end!
...
- ![|300](https://how-to-1258460161.cos.ap-shanghai.myqcloud.com/how-to202511230312662.png)
- 如何定义 $V^pi$ 呢? 下面这个公式更好，但是这需要从同一个 $s_t$ 出发多次采样，除了仿真以外是不可能的！
- 所以我们采用上面那个公式.
...
- Can we do better? 直接用 V 估计之后的收益，尽管一开始不准.
	- Lower variance, Higher bias! ("because it's using b hat instead of a single sample estimator", 或者说 $V(s_(t + 1))$ 是一个期望而不是一个采样. )
...
再次启动! https://www.bilibili.com/video/BV1NjH4eYEyZ?spm_id_from=333.788.videopod.sections&vd_source=e91cd5187b413becc0ce8b3bb56435c9&p=22
