...
1. 可以理解为：上述 "REINFORCE algorithm" 公式是对 $theta$ 求导，$theta$  必须是最新的，求的梯度才有意义。这导致训练效率很低. 当然你可以多采样几次，相当于 batch 大很多.
2. 或者理解为：采集的 (s, a, reward-to-go) 序列其实是 conditioned on policy 的. 比如 policy 现在的特性是走路径(1)收益很大，但是在历史轨迹中走 (1) 的收益很低，走路径 (2) 的收益很大，那么 PG 会学习走路径(2)，就学错了.
