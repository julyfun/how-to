...
flowchart TD
    ...
    SAC -->|"1. add V(s)，使用 expectile loss<br>2."| IQL["Implicit Q-learning"]
```
...
1. 有 Q, V, actor.
2. 好处：在仅有静态数据集的情况下，SAC 的 actor(s).rsample() 可能产生远离静态数据集的数据，会被 argmax 或者 multi-critic max 放大. 这里训练 value_net 的目的就是换 actor_loss.
