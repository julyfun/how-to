...
flowchart TD
    ...
    Double-DQN -->|"1. Add actor，loss = -Q(actor(s))<br>2. entropy loss 替代 ε<br>3. 悲观估计"| SAC["Soft Actor-Critic"]
    SAC -->|"1. add V(s)，使用 expectile loss<br>2. use V(s) in Q_loss<br>3. 启发式 actor_loss"| IQL["Implicit Q-learning<br>[offline]"]
    AC -->|"1. IS clipped[0.8, 1.2]<br>2. Entropy loss<br>3. KL(π_old ‖ π_new)"| PPO["PPO<br>[off-policy]"]
