...
```python
1. s, a, r, nxt_s, done = replaybuffer.sample()
...
# GPT 说道：expectile 关心的是上下两边的平方误差力矩怎么平衡.
...
3. actor_loss = -exp(beta * (target_critic(s, a) - value_net(s)).detach()) * actor(s).log_prob(a)
#             |<---------- advantage weight, no grad ---------------->|
#                     截断到 100. 动作优势大就提升数据集中该动作的概率.
```
...
```python
1. 纯模仿学习 flow_matching，同时进行如下:

2. 和 SAC 一样训练 critic(s, a).

3. noise = randn()
采样 obs, action.
action = one_step_actor(obs, noise)
distill_loss = mse(action, flow_matching(obs, noise)) # 必须是同一个 noise
q_loss = -critic(obs, action)

推理用 one_step_actor.
```

你可能会问：为什么还需要一个 flow_matching 来 distill 呢，fm 本来不就纯模仿数据集吗，可以不可以直接 mse(数据集)？答案是不行，one_step_actor 需要模仿 noise 条件下的 flow matching，
教程说是为了增强 expressiveness.
