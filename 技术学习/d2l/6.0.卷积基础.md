...
- nn.Conv2d(in_channels, out_channels, kernel_size) 下文 k_h, k_w 指这里 kernel_size
    
## 细节
- 卷积层中，输入通道到输出通道是全连接的。卷积核的形状是 `out_channel * in_channel * k_h * k_w`（与图像大小无关）这也是此 nn.Conv2d 的参数量
- 每个输入-输出通道 pair 拥有仅一个 $k_h times k_w$ 的二维卷积核（或者说每个输出通道拥有一个 `in_channel * k_h * k_w`  的三维卷积核）.
