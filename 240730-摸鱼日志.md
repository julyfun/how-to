...
- 每天顿悟一点点！[ok] enc-dec 和 dec-only 的 cls 原理
- [idea] 为什么 llm 不需要 chunk-based 但机器人模型普遍 chunk-based 呢？恐怕是因为 llm predict-and-sampled 就是它的 next condition，而机器人的 predict-and-sampled 无法无延迟无误差地在机器人上执行，next condition 必须观测，因此需要有一定分块。
