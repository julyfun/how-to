...
⭐️⭐️⭐️ 改进了 Training-time RTC 和预训练方式 | 👤 Xiaomi Robotics, Rui Cai | [🌐](https://robotics.xiaomi.com/xiaomi-robotics-0.html) | [📃 2602.12684](https://hjfy.top/arxiv/2602.12684) | [✨](https://www.alphaxiv.org/abs/2602.12684) | [📂](https://github.com/XiaomiRobotics/Xiaomi-Robotics-0) |

完整预训练分两步。先用约 2 亿 robot timesteps 和 8000 万 VL samples 按 6:1 混合训练 Qwen3-VL-4B-Instruct，并用 Choice Policies 让 VLM 预测多组 action chunk 和分数。然后冻结 VLM，将其最后 16 层 KV cache 接到 16 层 DiT。公开复现可直接从 Pretrain checkpoint 开始：每帧输入三路相机、语言和 32 维 state。再用 rectified flow 学 30 步 action chunk。异步训练以 50% 概率保留 1 到 6 个真值动作前缀，对后续 token 的 RoPE index 增加 10 后随机遮掉较早前缀。loss 按在线预测与真值的 L1 误差加权，推理做 5 步 Euler 积分并丢弃计算期间已经执行的前 N 步。

Training RTC 在毛巾异常抓取后会循环甩动，说明直接依赖动作前缀容易学成复制。完整预训练代码、2 亿 robot timesteps 和两项真机数据没有公开，所以现有材料能复现公开 benchmark 推理和自有数据后训练，不能从头复现论文模型。
