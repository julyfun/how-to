...
预训练分两步，1) 用 VL sample 微调 VLM 并从 query T*action 直接 1 个 proj 出 N 个 action chunk，仅反向传播 L1 loss 最小的 chunk. 同时训练一个 embedding -> [score] -> pred L1. 2) 冻结 VLM 训练 action. 后训练 training-time RTC 为了防止 copy 前缀动作，增加大误差的权重并缩短每帧 action 能够看见的前缀（Λ 形注意力），以及随机 mask 前缀.
