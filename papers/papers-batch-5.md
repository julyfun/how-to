...
直接把所有视频时间戳归一化到 0~1，离散化，先训 `predictor: (frame_i, frame_j, l) -> offset prob (CrossEntropy GT onehot offset)`，然后对于每一帧减去

方法利用成功轨迹的倒放来提供回归动作的负样本，并用悲观估计抑制分布外数据优势分数被高估的问题。但文章没有做 predict continuous offset 和 bin offset 的 ablation，也没有考虑长程任务 (frame_i, frame_j) 是否足以推断进度信息.
