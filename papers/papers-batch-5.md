...
本文希望一次 video denoise 后可以进行随机多次 action denoise. 本文基于 fastwam，首先添加了 6 帧历史观测，然后每次 action denoise 时，取相机当前 obs 生成 query，对 video 历史 kv 采用 attention pooling (i.e. cross attn) 来修正历史 kv 并提供给 action dit. 于是 video dit 和 action dit 可以并发运行，video dit 可以在任意时刻更新历史 kv，并且 action dit 不需要等待它.

对于解决 WAM 恐怖的 video dit 延迟来说，并发是一个不错的思路，部署也会更为简单. 即使 IDM，这种思路似乎可以延续，只不过需要验证该范式是否有效. 本文 Demo 只有双臂 picknplace.
