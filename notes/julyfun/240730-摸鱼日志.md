...
- [ssh, gui, x11, xquartz] 今天以如下 config，macos 本地 export DISPLAY=:0 后 `ssh xxx`，直接运行 `xclock` 可在 mac 端显示.
    - 需要在 XQuartz 菜单中，前往 Preferences → Security. 确保勾选 "Allow connections from network clients"
